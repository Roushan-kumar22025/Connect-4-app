...
const getPosition = (gameBoard, moveChecks) => {
    for (let check = 0; check < moveChecks.length; check++) {
      for (let i = 0; i <= moveChecks[check].max; i += moveChecks[check].step) {
        const positionsToCheck = moveChecks[check].indexes.map(index => i + index);
  
        if (positionsToCheck.every(pos => pos >= 0 && pos < gameBoard.length)) {
          const series = positionsToCheck.map(pos => gameBoard[pos]).join('');
  
          switch (series) {
            case "1110":
            case "2220":
              return positionsToCheck[3];
            case "1101":
            case "2202":
              return positionsToCheck[2];
            case "1011":
            case "2022":
              return positionsToCheck[1];
            case "0111":
            case "0222":
